...
namespace IDLTools
{
	...
	public class MetaNode
	{
		...
		protected XmlNode OpenXmlNode(string id, string type, int index)
		{
		...
		{
			XmlNode node = OpenXmlNode(id, "node", index);
		...
		private void SetValue(string id, string val, string type, int index)
		{
			try
			{
				XmlNode node = OpenXmlNode(id, type, index);
				node.InnerText = val;
			}
			catch{}
		}
		...
		*/
		...
		public Object GetValue(string id, int index) 
		{
			...
			try 
			{
				...
				string xmlValue = GetString (id, -1);
		...
		{
			XmlNode node = OpenXmlNode(id, "", index);
	...
	public class MetaData : MetaNode
	{
		...
		public void ReadFile(string file_path)
		{
			try
			{
				doc.Load(file_path);
				root = doc;
				root = OpenXmlNode("MetaData", "root",-1);//doc.LastChild;
			}
			catch{}
		}
		...
		private void Init()
		{
			try
			{
				...
				root = doc.CreateNode(XmlNodeType.Element, "MetaData", "");
