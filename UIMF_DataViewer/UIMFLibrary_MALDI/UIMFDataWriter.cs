...
namespace UIMFLibrary
{
	public class DataWriter
	{
        ...
        private bool m_GlobalParameterColumnsVerified = false;
        ...
        public void AddGlobalParameter(string parameterName, string parameterType, double parameterValue)
        {
            try
            {
                m_dbCommandUimf = m_dbConnection.CreateCommand();
                m_dbCommandUimf.CommandText = "Alter TABLE Global_Parameters Add " + parameterName.ToString() + " " + parameterType.ToString();
                m_dbCommandUimf.CommandText += " UPDATE Global_Parameters SET " + parameterName.ToString() + " = " + parameterValue.ToString();
                this.m_dbCommandUimf.ExecuteNonQuery();
                m_dbCommandUimf.Dispose();
            }
            catch
        ...
        protected void ValidateFrameParameterColumns()
        {
            if (!m_FrameParameterColumnsVerified)
            {
                ...
                if (!DataReader.TableHasColumn(m_dbConnection, "Frame_Parameters", "maldi_PosX"))
                    AddFrameParameter("maldi_PosX", "DOUBLE", 0);

                if (!DataReader.TableHasColumn(m_dbConnection, "Frame_Parameters", "maldi_PosY"))
                    AddFrameParameter("maldi_PosY", "DOUBLE", 0);

                if (!DataReader.TableHasColumn(m_dbConnection, "Frame_Parameters", "voltDefaultFragCh1"))
                    AddFrameParameter("voltDefaultFragCh1", "DOUBLE", 0);

                if (!DataReader.TableHasColumn(m_dbConnection, "Frame_Parameters", "voltDefaultFragCh2"))
                    AddFrameParameter("voltDefaultFragCh2", "DOUBLE", 0);

                if (!DataReader.TableHasColumn(m_dbConnection, "Frame_Parameters", "voltDefaultFragCh3"))
                    AddFrameParameter("voltDefaultFragCh3", "DOUBLE", 0);

                if (!DataReader.TableHasColumn(m_dbConnection, "Frame_Parameters", "voltDefaultFragCh4"))
                    AddFrameParameter("voltDefaultFragCh4", "DOUBLE", 0);

                if (!DataReader.TableHasColumn(m_dbConnection, "Frame_Parameters", "TIC_Count"))
                    AddFrameParameter("TIC_Count", "INT", 0);
        ...
        protected void ValidateGlobalParameterColumns()
        {
            if (!m_GlobalParameterColumnsVerified)
            {
                if (!DataReader.TableHasColumn(m_dbConnection, "Global_Parameters", "maldi_Width"))
                    AddGlobalParameter("maldi_Width", "DOUBLE", 0.0);

                if (!DataReader.TableHasColumn(m_dbConnection, "Global_Parameters", "maldi_Height"))
                    AddGlobalParameter("maldi_Height", "DOUBLE", 0.0);

                m_GlobalParameterColumnsVerified = true;
            }
        }
